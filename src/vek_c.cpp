...
int vek_authority_validate_request(vek_runtime*r,const char*action,const char*actor,uint64_t sequence,const char*nonce,const char*payload,const char*cap,double now){if(!r||!action||!actor)return 0;vek::CapabilityManifest caps;if(cap&&cap[0])caps.Grant(cap);caps.Seal();vek::AuthorityRequest q;q.actionId=action;q.actorId=actor;q.sequence=sequence;q.nonce=nonce?nonce:"";q.payload=VekValue(payload?payload:"");auto d=r->authority.ValidateClientRequest(q,r->actions,caps,(float)now);r->authority_reason=d.reason;return d.allowed?1:0;}
