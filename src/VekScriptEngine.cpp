...
bool VekScriptEngine::RegisterNative(const std::string&n,NativeFunction f){if(impl->nativesSealed){impl->lastError="VEK security: native registry is sealed";return false;}impl->natives[n]=std::move(f);return true;}void VekScriptEngine::SealNativeRegistry(){impl->nativesSealed=true;}bool VekScriptEngine::NativeRegistrySealed()const{return impl->nativesSealed;}
...
void VekRegisterStandardLibrary(VekScriptEngine&e){
    ...
    e.RegisterNative("array_push",[](const std::vector<VekValue>&a){if(a.size()<2||!a[0].IsArray())return VekValue(false);VekValue x=a[0];x.Push(a[1]);return VekValue(true);});
    ...
    e.RegisterNative("map_set",[](const std::vector<VekValue>&a){if(a.size()<3||!a[0].IsMap())return VekValue(false);VekValue x=a[0];return VekValue(x.Set(a[1].AsString(),a[2]));});
